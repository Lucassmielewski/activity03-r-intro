...
**Response**: The upper_case vector was missing a comma after "H". The punctuation vector was using double quotes to denote a double quote as a string which stopped it from reading it correctly, replaced with single quotes.
...
**Response**: Cbind takes the three vectors and makes them into a matrix rather than combining them into a longer vector. 
...
**Response**: The names function needs an index to turn the first column into Symbol. Also data.frame() not dataframe()
...
**Response**: Find the length of the column in the dataframe not the dataframe itself
